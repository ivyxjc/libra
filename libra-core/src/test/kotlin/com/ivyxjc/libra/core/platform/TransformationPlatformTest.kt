...
class TransformationPlatformTest {
    ...
    fun testBlockedRetryPlatformException() {
        val transformations =
                Transformation(listOf(ProcessorsContainer.get(ProcessorPattern.Builder().retryType(RetryType.PLATFORM).sync(0).build()),
        ...
        try {
            ...
            Assert.assertEquals(6, ucTxn.platformAttempt)
        }
    ...
    fun testAsyncInMemoryRetryPlatformException() {
        val transformations =
                Transformation(listOf(ProcessorsContainer.get(ProcessorPattern.Builder().retryType(RetryType.PLATFORM).sync(2).build())
                ))
        val sourceConfig = sourceConfigBuilder.transformation(transformations).build()
        Mockito.`when`(sourceConfigService.getSourceConfig(1)).thenReturn(sourceConfig)
        try {
            ...
            Assert.assertEquals(1, ucTxn.platformAttempt)
        }
    }
}
